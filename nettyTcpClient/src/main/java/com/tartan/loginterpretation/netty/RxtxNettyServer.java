...
import com.tartan.loginterpretation.netty.handler.ExceptionHandler;
import com.tartan.loginterpretation.netty.handler.HeartBeatServerHandler;
            ...
            b.group(group)
                    .channelFactory(new ChannelFactory<RxtxChannel>() {
                        ...
                        protected void initChannel(RxtxChannel ch) throws Exception {
                            ...
                            //添加编解码和处理器(节点间通讯用)
                            chPipeLine.addLast("rxtxBdeCoder", nettyDecoder);
                            //WITS数据处理
                            chPipeLine.addLast(new IdleStateHandler(10, 0, 0, TimeUnit.SECONDS));
                            chPipeLine.addLast("rxtxIn", nettyInHandler);
                            //心跳检测
                            chPipeLine.addLast("rxtxHeartBeat", heartBeatServerHandler);
                            //异常处理
                            chPipeLine.addLast("rxtxException", exceptionHandler);
                        }
                    });

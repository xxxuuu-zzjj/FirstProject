...
class P2028_FindMissingObservations {
    ...
    class Solution {
        public int[] missingRolls(int[] rolls, int mean, int n) {
            int times = rolls.length + n;
            int sumWithN = mean * times - Arrays.stream(rolls).sum();
            int[] rollsN = new int[n];
            if (n * 6 < sumWithN || n > sumWithN) {
                return new int[0];
            }
            for (int i = 1; i <= 6; i++) {
                if (sumWithN >= i * n && sumWithN < n * (i + 1)) {
                    rollsN[0] = i;
                    break;
                }
            }
            int surplus = sumWithN - n * rollsN[0];
            for (int i = 1; i < n; i++) {
                rollsN[i] = rollsN[0];
                if (surplus > 0){
                    rollsN[i]++;
